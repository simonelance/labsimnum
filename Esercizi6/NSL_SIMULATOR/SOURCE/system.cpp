...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){
    ...
    // io avrò un certo valore di probabilità di ottenere lo spin up. la somma dei valori di spin assumibili può fare +2, 0, -2;
    ...
    if(_sim_type == 1){       // LJ system
      ...
      _particle(i).translate(shift, _side);  //Call the function Particle::translate _side useful for periodic bound conditions
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp( path + "potential_energy.dat");  // " + flag + " , me lo tengo in caso volessi cambiare i nomi files per ciascuna temp
        ...
        coutt << "TEMPERATURE:   ACTUAL_TE:    TE_AVE:      ERROR:" << endl;
        coutt.close();
        _nprop++;
        ...
        coutpr << "TEMPERATURE:   ACTUAL_M:     M_AVE:       ERROR:" << endl;
        coutpr.close();
        _nprop++;
        ...
        cout << "ATTENZIONE: per calcolare cv devi anche calcolare la total_energy!" << endl;// se ti da index out of bound l'errore è questo.
        coutpr << "TEMPERATURE:   ACTUAL_CV:    CV_AVE:      ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_cv = true;
        ...
        coutpr << "TEMPERATURE:   ACTUAL_X:     X_AVE:       ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_chi = true;
...
void System :: read_configuration(){
  ifstream cinf;
  (_sim_type <= 1) ? cinf.open("../INPUT/CONFIG/config.xyz") : cinf.open("../INPUT/CONFIG/config.ising") ;// ? -se viene risp la condizione- : -cosa fare quando non è rispettata-
  ...
  if(_restart and _sim_type > 1){
    int spin;
    cout << endl <<"Parametro restart =1, leggo la precedente configurazione di spin salvata dopo l'equilibrazione."<< endl;
    if(_sim_type == 2){cinf.open("../OUTPUT/CONFIG/equilibrated/config.spin");
    }else { cinf.open("../OUTPUT/CONFIG/equilibrated_gibbs/config.spin");}
    for(int i=0; i<_npart; i++){
      cinf >> spin;
...
void System :: temp_reset(double temp){ // Reset block accumulators to zero
  ofstream coutf;
  if(temp>0){
    coutf.open("../OUTPUT/output.dat",ios::app);
    coutf <<endl << "-- TEMPERATURA"<< temp << " COMPLETATA -- "<< endl;
    coutf.close();
  }
  _average.zeros();
  _global_av.zeros();
...
void System :: measure(){ // Measure properties
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy or _measure_temp or _measure_pressure or _measure_cv or _measure_tenergy){
  ...
  if (_measure_magnet or _measure_chi){
    for(int i=0; i<_npart; i++){  magnet_temp += double( _particle(i).getspin());}
    if (_measure_magnet) _measurement(_index_magnet)= magnet_temp;
  }


  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv){
     _measurement(_index_cv) = tenergy_temp* tenergy_temp * double(_npart)*double(_npart); //mi salvo a questo indice i valori dei quadrati dell'energia in modo che una volta terminato il blocco io possa mediarlo sul numero di step ed ottnere <H^2>
  }

  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if(_measure_chi) {
    _measurement(_index_chi) = _beta * magnet_temp * magnet_temp/ double(_npart) ;// non divido per il numero di particelle perche mi chiede sucettibilita totale del sistemA
  }
...
void System :: averages(int blk , const string flag , const string path){
  ...
  //quindi in pratica in quest ultimo sto facendo
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open( path + "potential_energy" + flag + ".dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy && blk == _nblocks){
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet && blk== _nblocks){
    coutf.open( path + "magnetization.dat",ios::app);
  ...
  // TO BE FIXED IN EXERCISE 6
  if (_measure_cv && blk== _nblocks){
    coutf.open( path + "specific_heat.dat",ios::app);
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi && blk == _nblocks){
    coutf.open( path + "susceptibility.dat",ios::app);
...
void System :: Update_temp(double t){
  _temp = t;
  _beta = 1/_temp;
}

void System :: Set_restart(bool x){
  _restart = x;
