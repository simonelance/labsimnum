...
void System :: Equilibrate(int n_eq){  

  for(int j = 0; j < n_eq ; j++) this->step();
}
...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){
    //Gibbs sampler for Ising----------------------------- FUNZIONA
    double delta_E, p_spinup;
    //quando lo stato di spin finale ha spin +1 => il segno dell'esponente è negativo
    delta_E = 2.0 * ( _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H );
    // in questo algoritmo io so la probabilita codizionat. in base ai valori di spin delle due particerre affinaco alla iesima
    // io avroò un certo vlore di probabilità di ottenere lo spin up. la somma dei valori sin assumibili può fare +2, 0, -2;
    // in base a questi valori p_spinup assume valori di prbabilità che lo spin della iesima particella sia up. ovviamente 1-p_spinup
    //rappresenta p_spindown
    p_spinup = 1/( 1 + exp(- _beta*delta_E));
    if( _rnd.Rannyu() < p_spinup) _particle(i).setspin(+1);
    else _particle(i).setspin(-1); // questo algoritmo "non rifiuta mai", ma l'assegnare lo spin down è equivalente ad un refuse dello spin up
  } else {           // M(RT)^2
    if(_sim_type == 1){       // LJ system
      vec shift(_ndim);       // perogni passo useremo questo vec tridimensionale per attuare una traslazione della nostra particella
      for(int j=0; j<_ndim; j++){
        shift(j) = _rnd.Rannyu(-1.0,1.0) * _delta; // spostamento in ognuna delle 3 dimensioni uniform distribution in [-_delta;_delta)
      }
      _particle(i).translate(shift, _side);  //Call the function Particle::translate _side  serve per le pbc
      if(this->metro(i)){ //Metropolis acceptance evaluation, etro mi restituisce un booleano vero se mossa accettata falso altrimenti
        _particle(i).acceptmove();
        _naccepted++;
      } else _particle(i).moveback(); //If translation is rejected, restore the old configuration, praticamente è la mossa inversa di translate.
    } else {                  // Ising 1D -----------------------------------------------------------------
...
bool System :: metro(int i){ // Metropolis algorithm
  ...
  if(_sim_type == 1) delta_E = this->Boltzmann(i,true) - this->Boltzmann(i,false); //La funzione boltzmann ci serve nel momento in cui le particelle si muovono e quindi cambiano le varie interazioni reciproche
  else delta_E = 2.0 * _particle(i).getspin() * 
                 ( _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H );
                 //è la formuletta che troviamo a pag 21 delle slides. H penso sia il termine di interazione con un eventuale campo magnetico
  acceptance = exp(-_beta*delta_E); // Questo è il rapporto tra le distribuzioni di prob di boltzmann 
...
double System :: Boltzmann(int i, bool xnew){
  ...
  for (int j=0; j<_npart; j++){
    if(j != i){
      ...
      dr = sqrt(dr);
      if(dr < _r_cut){
        energy_i += 1.0/pow(dr,12) - 1.0/pow(dr,6);  // utilizza il potenziale di lennard jones
      }
    }
  }
  return 4.0 * energy_i;
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      if(_sim_type == 0){      coutf << "LJ MOLECULAR DYNAMICS (NVE) SIMULATION"  << endl; _flag="_LJ_MD";} //LJ_MD
      else if(_sim_type == 1) {coutf << "LJ MONTE CARLO (NVT) SIMULATION"         << endl; _flag="_LJ_MC";}
      else if(_sim_type == 2) {coutf << "ISING 1D MONTE CARLO (MRT^2) SIMULATION" << endl; _flag="IS_MTR";}
      else if(_sim_type == 3) {coutf << "ISING 1D MONTE CARLO (GIBBS) SIMULATION" << endl; _flag="IS_GIB";}
...
void System :: initialize_velocities(){
  ...
  if(_restart){ // se restart è 1 io riprendo da dove la mia simulazione era finita, percui se avevo equilibrato il sistema e metto =1 continuo col sistema equilibrato
    ifstream cinf;
    ...
    // same thing for file config.xyz. this way I can get the values of the velocities of the last simulation
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp(path + "potential_energy" + _flag + ".dat");
        ...
        index_property++;
        _vtail = (8*M_PI*_rho/(9*pow(_r_cut,9))) - (8*M_PI*_rho/(3*pow(_r_cut,3))); // TO BE FIXED IN EXERCISE 7.1
      } else if( property == "KINETIC_ENERGY" ){
        ofstream coutk(path + "kinetic_energy" + _flag + ".dat");
        ...
        ofstream coutt(path + "total_energy" + _flag + ".dat");
        ...
        ofstream coutte( path + "temperature" + _flag + ".dat");
        ...
        ofstream coutpr(path + "pressure" + _flag + ".dat");
        ...
        index_property++;
        _ptail = 32*M_PI*_rho*(1/(9*pow(_r_cut,9))-1/(6*pow(_r_cut,3))); // TO BE FIXED IN EXERCISE 7.1
      } else if( property == "GOFR" ){
        ofstream coutgr(path + "gofr" + _flag + ".dat");
        ...
        ofstream coutpv(path + "pofv" + _flag + ".dat");
        ...
        ofstream coutpr(path + "magnetization" + _flag + ".dat");
        ...
        ofstream coutpr(path + "specific_heat" + _flag + ".dat");
        ...
        ofstream coutpr(path + "susceptibility" + _flag + ".dat");
...
void System :: write_configuration(){
  ...
  if(_sim_type < 2){
    ...
    if(coutf.is_open()){
      ...
      for(int i=0; i<_npart; i++){
        coutf << "LJ" << "  " 
              << setprecision(17) << _particle(i).getposition(0,true)/_side(0) << "   " // x scrivuamo le posizioni all'ultimo passo a cui siamo arrivati
    ...
    if(coutf.is_open()){
      ...
      for(int i=0; i<_npart; i++){
        coutf << "LJ" << "  "
              << setprecision(17) << _particle(i).getposition(0,false)/_side(0) << "   " // x getposition con argomento false mi restituisce la posrizione vecchia.
    ...
    coutf.close();
    //this->write_velocities(); // saving final velocities in a separated file. this i can resume the simulation with the right veloities after system equilibration
  } else {
...
void System :: write_velocities(){
  ofstream coutf;
  coutf.open("../OUTPUT/CONFIG/velocities.xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
      coutf << "LJ" << "  " 
            << setw(16) << _particle(i).getvelocity(0)          // x
            << setw(16) << _particle(i).getvelocity(1)          // y
            << setw(16) << _particle(i).getvelocity(2) << endl; // z
    }
  } else cerr << "PROBLEM: Unable to open config.xyz" << endl;
  coutf.close();
  return;
...
void System :: read_configuration(){
  ifstream cinf;
  if(_sim_type <=1) cinf.open("../INPUT/CONFIG/config.fcc");
...
void System :: measure(){ // Measure properties
  ...
  // KINETIC ENERGY //////////////////////////////////////////////////////////// en cinetica per particella
  if (_measure_kenergy or _measure_temp or _measure_tenergy){
  //if (_measure_kenergy or _measure_temp or _measure_pressure or _measure_cv){
    if(_sim_type ==0) for (int i=0; i<_npart; i++) kenergy_temp += 0.5 * dot( _particle(i).getvelocity() , _particle(i).getvelocity() ); 
    else if(_sim_type ==1) kenergy_temp = _npart * 0.5 *  3. * _temp;  // sarebbe energia considerando la vel quadratica media delle particelle di un gas come 3t
    kenergy_temp /= double(_npart);
    if(_measure_kenergy) _measurement(_index_kenergy) = kenergy_temp; //l'if qua serve perche altrimenti nel momento in cui volessi calcolare la temperatura ma non la kinetik_e starei salvando il valore in uno spazio del vettore non esistente
  }
  // TOTAL ENERGY (kinetic+potential) //////////////////////////////////////////
  if (_measure_tenergy or _measure_cv){ //tenergy sta per total energy del mio sistema di spin
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp;
    else { 
      double s_i, s_j;
      for (int i=0; i<_npart; i++){
        s_i = double(_particle(i).getspin());
        s_j = double(_particle(this->pbc(i+1)).getspin());  //pbc(int ) di fatto prende il numero delle particelle , e se con i+1 si sfora _npart ti riporta alla particella 1
        tenergy_temp += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j); // calcolo brute force
      }
      tenergy_temp /= double(_npart); // calcolo energia totale e la medio su npart per trovare un energia per particella
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet or _measure_chi) for(int i=0; i<_npart; i++) _measurement(_index_magnet) += double( _particle(i).getspin());

  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv) _measurement(_index_cv) = tenergy_temp* tenergy_temp; //mi salvo a questo indice i valori dei quadrati dell'energia in modo che una volta terminato il blocco io possa mediarlo sul numero di step ed ottnere <H^2>

  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if(_measure_chi) {
    _measurement(_index_chi) = _beta * _measurement(_index_magnet)*_measurement(_index_magnet);// non divido per il numero di particelle perche mi chiede sucettibilita totale del sistemA
  }
...
void System :: averages(int blk, const string path){
  ...
  if(_measure_cv){ // sotto index_cv sono contenute le somme quadrate delle energie per particella
    _block_av(_index_cv) = pow(_beta,2) * (_block_av(_index_cv) - _block_av(_index_tenergy)* _block_av(_index_tenergy)/double(_nsteps) );
  } // mi lascio una divisione per _nsteps in meno appodsta per farla insieme a tutte le altri nel passaggio sotto
 

  _average     = _block_av / double(_nsteps); // qualsiasi grandezza ha accumulato valorie in block_av per tutti gli step quindi devo fare la divisione per ottenere un valore madiato.
  _global_av  += _average; // sto facendo una somma elemento per elemento, in modo da accumi
  _global_av2 += _average % _average; // % -> element-wise multiplication: eleva al quadrato ogni elemento del vettore moltiplicando ogni componente x se stessa
  //quindi in pratica quest ultimo vettore mi servirà nel calcolo degli errori sulla media blocchi e basta.

  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open(path + "potential_energy" + _flag + ".dat",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open(path + " kinetic_energy" + _flag + ".dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open(path + "total_energy" + _flag + ".dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open(path + "temperature" + _flag + ".dat",ios::app);
  ...
  if (_measure_pressure){
    coutf.open(path + "pressure" + _flag + ".dat",ios::app);
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr && _nblocks == blk){ // ovvero stampami solo le misure dell'ultio blocco in cui ho i valori finali
    coutf.open(path + "gofr" /*+ std::to_string(blk)*/  + _flag + ".dat",ios::app);
      for(int i=0; i<_n_bins ; i++ ){
        double d= (i + 0.5) * _bin_size; // stampo su file posizione media di ciascun blocco
...
//              << setw(12) << average
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open(path + "acceptance.dat",ios::app);
