...
// TO BE FIXED IN EXERCISE 6
  ...
  _block_av += _measurement; //Accumulo le varie (energie/pressioni/conteggi velocità) medie ad ogni passo

  return;
}

void System :: averages(int blk){ //Averages inizia una volta terminati gli _nsteps del blocco in questione
  ...
  _global_av  += _average;   //  accumulo le medie temporali di ciascun blocco per poi ottenere la media cumulativa dividendo sul numero di blocchi
  _global_av2 += _average % _average; // % -> element-wise multiplication: ovvero sto ottenendo componente i esima^2 
  ...
  if (_measure_pofv){
    ...
    for(int i=0; i<_n_bins_v; i++){
      ...
      coutf << setw(12) << (i +0.5) * _bin_size_v // in mezzo al bin
