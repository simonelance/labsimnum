...
void System :: Verlet(){
  ...
  for(int i=0; i<_npart; i++){ //Force acting on particle i
    _fx(i) = this->Force(i,0);  // this è il puntatore che ci dice che stiamo puntando ad una funzione della classe in cui implementiamo il codice
    _fy(i) = this->Force(i,1);  // è soprattutto utile nel momento in cui ci siano ambiguità (ad esempio la funzione pbc è definita sia in particle che in system)
    _fz(i) = this->Force(i,2);  // in questo modo chiariamo al compilatore che stiamo richiamando la funzione interna alla classe
  }
  for(int i=0; i<_npart; i++){ //Verlet integration scheme
    xnew = this->pbc( 2.0 * _particle(i).getposition(0,true) - _particle(i).getposition(0,false) + _fx(i) * pow(_delta,2), 0); // la funzione getposition mi restituisce la posizione attuale se le do true in pasto, altrimenti mi restituisce la posizione allo step precedente
    ...
    _particle(i).setvelocity(0, this->pbc(xnew - _particle(i).getposition(0,false), 0)/(2.0 * _delta)); //this->pbc(xnew - _particle(i).getposition(0,false), 0)/(2.0 * _delta)
    _particle(i).setvelocity(1, this->pbc(ynew - _particle(i).getposition(1,false), 1)/(2.0 * _delta)); // questa è gia la velocita calcolata tramite la posizione con l'algoritmo
    _particle(i).setvelocity(2, this->pbc(znew - _particle(i).getposition(2,false), 2)/(2.0 * _delta)); // di verlet.
...
void System :: initialize(string path){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ofstream couta( path + "acceptance.dat"); // Set the heading line in file ../OUTPUT/acceptance.dat
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input("../INPUT/input.dat"); // Start reading ../INPUT/input.dat
  ofstream coutf;
  coutf.open(path + "output.dat");
  string property;
  double delta;
  while ( !input.eof() ){
    input >> property;
    if( property == "SIMULATION_TYPE" ){
      ...
      input >> _restart;  // restart è inizializzato a false
    ...
    } else if( property == "PUNTO" ){
      input >> _punto;
      coutf << "PUNTO= " << _punto << endl;
      if(_punto !=2) {cerr << endl << "Be Careful, you need PUNTO =2 for exercise 4.2 and 4.3!"<< endl ; exit(-2);}
...
void System :: initialize_velocities(){
  ...
  if(_restart){ //al primo passo restart è inizializzato a zero, quindi non si entra in questa parentesi. Nel nostro codice per ora non entriamo mai in questo if perche restart=0 
    ifstream cinf;
    ...
    cinf.close();
  } else { //IMPLEMENTAZIONE VELOCITA DI PARTENZA DISTRIBUITE GAUSSIANAMENTE ------------------------------------------------------------------------
    vec vx(_npart), vy(_npart), vz(_npart);
      ...
      for (int i=0; i<_npart; i++){
        ...
        }else if(_punto == 2 ) { // IMPLEMENTAZIONE DELLA DISTRIBUZIONE DI VELOCITÀ COME UNA DELTA DI DIRAC --------------------------------------------------------------------
          vx(i)=0.;
          vy(i)=0.;
          vz(i)=0.;
          // punto =2 è la condizione che i fa entrare in questo tipo di inizializzazione delle velocità
        int componente = int(_rnd.Rannyu(0,3));
        int verso = (int(_rnd.Rannyu(0,2)) *2) -1;

        if(componente==0 )       { vx(i)= double(verso*sqrt(3* _temp));} // una sola delle tra cambia, le altre due rimangono a 0
        else if(componente == 1 ){ vy(i)= double(verso*sqrt(3* _temp));}
        else if(componente == 2 ){ vz(i)= double(verso*sqrt(3* _temp));}
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp(path + "potential_energy.dat");
        ...
        ofstream coutk(path + "kinetic_energy.dat");
        ...
        ofstream coutt(path + "total_energy.dat");
        ...
        ofstream coutte(path + "temperature.dat");
        ...
        ofstream coutpr(path + "pressure.dat");
        ...
        ofstream coutpv(path + "pofv.dat");
        coutpv << "VELOCITY:     AVE_POFV:    PROG_AVE:        ERROR:" << endl;
        coutpv.close();
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        ...
        ofstream coutpr(path + "magnetization.dat");
        ...
        ofstream coutpr(path + "specific_heat.dat");
        ...
        ofstream coutpr(path + "susceptibility.dat");
        ...
        ofstream coutf;
        coutf.open(path + "output.dat",ios::app);
...
void System :: write_configuration(string path){ // contenuto nella funzione finalize sopra
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open(path + "CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.close();
    coutf.open(path + "CONFIG/conf-1.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
        coutf << "LJ" << "  "
              << setprecision(17) << _particle(i).getposition(0,false)/_side(0) << "   " // x , il parametro false ci restituisce la vecchia posiz della particella
    ...
    coutf.open(path + "CONFIG/config.spin");
...
void System :: write_XYZ(int nconf, string path){
  ofstream coutf;
  coutf.open(path + "CONFIG/config_" + to_string(nconf) + ".xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: read_configuration(){
  ifstream cinf;
  if (_sim_type < 2 and _punto == 2){
      cout << "Initializing particles inside the box with half of the sides" << endl;
...
void System :: block_reset(int blk, string path){ // Reset block accumulators to zero
  ofstream coutf;
  if(blk>0){
    coutf.open(path + "output.dat",ios::app);
...
void System :: measure(){ // Measure properties
  ...
  double virial=0.0;
  if (_measure_penergy or _measure_pressure or _measure_gofr) { // ti mette questa condizione se vale almeno una di esse, poi dopo mette condizione in base a quale è vera
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){ // itera sulle particelle e si calcola la distanza reciproca tra esse
        distance(0) = this->pbc( _particle(i).getposition(0,true) - _particle(j).getposition(0,true), 0);
        distance(1) = this->pbc( _particle(i).getposition(1,true) - _particle(j).getposition(1,true), 1);
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt( dot(distance,distance) ); // distanza in 3d tra due particelle
        // GOFR ... TO BE FIXED IN EXERCISE 7
        if(dr < _r_cut){ 
          if(_measure_penergy)  penergy_temp += 1.0/pow(dr,12) - 1.0/pow(dr,6); // POTENTIAL ENERGY: di fatto calcola ogni volta il potenziale tra due particelle diverse e lo somma cumulativamente per calcolare en.pot. totale del sistema
          if(_measure_pressure) virial       += 1.0/pow(dr,12) - 0.5/pow(dr,6); // PRESSURE: idem per la pressione
        }
      }
    }
  }
  ...
  if (_measure_pofv){
    for (int i=0; i<_npart; i++) { // dopo lo step, calcolo il modulo della velocita delle particelle e le divido nei vari bin 
      ...
      bin = int(vel/ _bin_size_v); // ogni volta ridefinisco il bin tramite la parte intera di questo quoziente in modo da aggiungere un nuovo termine al contatore.
      //occhio perchè bin è gia stato diciarato a inizio funzione e ridichiararlo potrebbe causare dei conflitti.
      if( bin < _n_bins_v ){  
        _measurement(_index_pofv + bin) += (1./_npart) ;        // in questo modo nel main stiamo ottenendo ad ogni step, ovvero ad ogni iterazione nel tempo le grandezze medie.
      }
    }
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    penergy_temp = _vtail + 4.0 * penergy_temp / double(_npart); // arrvato a questo punto si calcola la penergy media. Infatti prende la potenziale cumulata e la divide per npart.
    _measurement(_index_penergy) = penergy_temp;        // in questo modo nel main stiamo ottenendo ad ogni step, ovvero ad ogni iterazione nel tempo le grandezze medie.
  }
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    for (int i=0; i<_npart; i++) kenergy_temp += 0.5 * dot( _particle(i).getvelocity() , _particle(i).getvelocity() ); //energia cinetica con massa unitaria
...
// TO BE FIXED IN EXERCISE 6
    ...
    // in measurement che è un vettore io ho un indice fisso per ciascuna proprietà (pressione/cinetica/potenziale) e mi salva i valori di energia media
  _block_av += _measurement; //Accumulo i conteggi dei bin per tutti i passi di ogni blocco. _measurement
...
void System :: averages(int blk, string path){
  ...
  _average     = _block_av / double(_nsteps); // media temporale del blocco per ciascuna delle componenti del vettore 
  _global_av  += _average/ blk;   //  accumulo le medie temporali di ciascun blocco per poi ottenere la media cumulativa dividendo sul numero di blocchi
  _global_av2 += (_average % _average)/ (blk*blk); // % -> element-wise multiplication: ovvero sto ottenendo componente i esima^2 


  //  VELOCITY DISTRIBUTION FOR HISTOGRAM //////////////////////////////////////////////////////////
  if (_measure_pofv){
    coutf.open(path + "pofv.dat",ios::app);
    for(int i=0; i<_n_bins_v; i++){
      ...
      coutf << setw(12) << (i +0.5) * _bin_size_v
            << setw(12) << average// quindi io qua sto stampando per ogni bin il conteggio in quel bin
            << setw(12) << sum_average/ double(blk) // normalizzo il conteggio della somma per il numero di blocchia cui sono arrivato
            << setw(12) << this->error( sum_average , sum_ave2 , blk) << endl; // error mi calcola l'errore sui valori accumulati fino al blocco numero blk che cli passo in input
    }
    coutf.close();
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open(path + "potential_energy.dat",ios::app);
    average  = _average(_index_penergy); // in questo file salvo solo la potential energy, quindi seleziono solo la componente che corrisponde ad essa
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open(path + "kinetic_energy.dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open(path + "total_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open(path + "temperature.dat",ios::app);
  ...
  if (_measure_pressure){
    coutf.open(path + "pressure.dat",ios::app);
...
int System :: get_indexpofv(){
  return _index_pofv;
}

double System :: get_binsizev(){
  return _bin_size_v;
}

vec System :: get_measurement(){
  return _measurement;
}
