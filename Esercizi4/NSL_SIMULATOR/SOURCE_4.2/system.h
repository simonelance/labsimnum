...
private:
  ...
  int _punto =0;        //mi determina il sottopunto degli esercizi a cui sono arrivato
  ...
  int _index_pofv;                                          // Index for accessing velocity modulus distribution------- quello che serve a noi per implementare la distribuzione
...
public: // Function declarations
  ...
  void initialize(string);          // Initialize system properties
  void initialize_properties(string);// Initialize properties for measurement
  void finalize(string);            // Finalize system and clean up
  void write_configuration(string); // Write final system configuration to XYZ file
  void write_XYZ(int nconf,string);  // Write system configuration in XYZ format on the fly
  ...
  void block_reset(int blk, string);  // Reset block averages
  void measure();             // Measure properties of the system
  void averages(int blk, string );     // Compute averages of properties
